...
// console.log("a content data:",contentData,content);
 const file2Base64 = (file: File): Promise<string> => {
    ...
    (files: File[], insertPosition?: number): void => {
      ...
      Promise.all(
        files.map(async (file) => {
          ...
          const uploadFile=await apiRemote.uploadFile(file);
          return {
            src:  `${process.env.STORAGE_CONTEST_URL}${uploadFile.url}`,
            alt: file.name,
          };
        })
      ).then((images)=>{
        // console.log("a images:",images);
        insertImages({
          images: images,
          editor: rteRef?.current?.editor as any,
  ...
  const editorUpdateContent=()=>{
    var html=rteRef.current?.editor?.getHTML() ?? "";
  ...
  useEffect(()=>{
    ...
    // if (rteRef.current){
    //   rteRef.current?.editor?.on('update',editorUpdateContent);
    //   return () => {
    //     rteRef.current?.editor?.off('update',editorUpdateContent);
    //   }
    // }
  ...
  useEffect(()=>{
    if(contentData){
      if (onChange) onChange(contentData);
    }
  },[contentData])
